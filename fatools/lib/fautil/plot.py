...
def bp_plotrange(fsa, plotrange):
    import numpy as np
    fit = np.poly1d(fsa.z)

    left = 1000
    while (fit(left) < plotrange[0]):
        left += 20
    left -= 20

    right = 6000
    while (fit(right) > plotrange[1]):
        right -= 20
    right += 20
    return (left, right)


def do_plot(fsa, plot_file=None, plotrange=None, plotlist=['all']):
    """
    ...
    for channel in channels:
        ...
        if color not in plotlist and 'all' not in plotlist:
            continue
    ...
    if plotrange:
        if plotrange[0] < 1000:
            # convert from s.t.u. to b.p.
            plotrange = bp_plotrange(fsa, plotrange)
        plt.xlim(*plotrange)
...
def command_block(args, fsas, plot_file):
    ...
    if args.ladder_plot:
        ...
        for fsa in fsas:
            if args.plot:
                do_plot(fsa, plot_file, args.plotrange, args.plotlist)
            if args.split_plot:
